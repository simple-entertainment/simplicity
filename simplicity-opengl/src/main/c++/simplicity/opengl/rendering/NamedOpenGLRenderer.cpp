...
namespace simplicity
{
  namespace opengl
  {
    ...
    void
    NamedOpenGLRenderer::dispose()
    {
      glPointSize(1.0f);
    }

    Renderer::DrawingMode
    NamedOpenGLRenderer::getDrawingMode() const
    {
      return (fDrawingMode);
    }

    int
    NamedOpenGLRenderer::getOpenGlDrawingMode(const DrawingMode drawingMode)
    {
    ...
    void
    NamedOpenGLRenderer::init()
    {
      glPointSize(2.0f);
    }

    void
    NamedOpenGLRenderer::renderModel(const GLUCapsule& capsule)
    {
    ...
    void
    NamedOpenGLRenderer::renderModel(const GLUCylinder& cylinder)
    {
    ...
    void
    NamedOpenGLRenderer::renderModel(const GLUSphere& sphere)
    {
    ...
    void
    NamedOpenGLRenderer::renderModel(const GLUTorus& torus)
    {
    ...
    void
    NamedOpenGLRenderer::renderModel(const IndexedVectorVG& vertexGroup)
    {
    ...
    void
    NamedOpenGLRenderer::renderModel(const Model& model)
    {
      renderModel(model, 0);
    }

    void
    NamedOpenGLRenderer::renderModel(const Model& model, const int name)
    {
      glPushName(name);
    ...
    void
    NamedOpenGLRenderer::renderModel(const VectorVG& vertexGroup)
    {
    ...
    void
    NamedOpenGLRenderer::renderVectorVGEdges(const VectorVG& vertexGroup)
    {
      const vector<float>& colours = vertexGroup.getColours();
      const vector<float>& normals = vertexGroup.getNormals();
      const vector<float>& vertices = vertexGroup.getVertices();
      ...
      for (unsigned int index = 0; index < vertices.size() / ITEMS_IN_CNV - 1; index++)
      {
        ...
        {
          glColor3f(colours.at(index * ITEMS_IN_CNV), colours.at(index * ITEMS_IN_CNV + 1), colours.at(index * ITEMS_IN_CNV + 2));
          glNormal3f(normals.at(index * ITEMS_IN_CNV), normals.at(index * ITEMS_IN_CNV + 1), normals.at(index * ITEMS_IN_CNV + 2));
    ...
    void
    NamedOpenGLRenderer::renderVectorVGFaces(const VectorVG& vertexGroup)
    {
      const vector<float>& colours = vertexGroup.getColours();
      const vector<float>& normals = vertexGroup.getNormals();
      const vector<float>& vertices = vertexGroup.getVertices();
    ...
    void
    NamedOpenGLRenderer::renderVectorVGVertices(const VectorVG& vertexGroup)
    {
      const vector<float>& colours = vertexGroup.getColours();
      const vector<float>& normals = vertexGroup.getNormals();
      const vector<float>& vertices = vertexGroup.getVertices();
    ...
    void
    NamedOpenGLRenderer::setDrawingMode(const DrawingMode mode)
    {
      fDrawingMode = mode;
    }
  }

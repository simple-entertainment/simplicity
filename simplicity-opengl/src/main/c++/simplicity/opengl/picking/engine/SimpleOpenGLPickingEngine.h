...
namespace simplicity
{
  namespace opengl
  {
    /**
     * <p>
     * Manages the picking of a {@link simplicity::Scene Scene} in a OpenGL environment. This implementation uses only simple picking techniques and
     * properties.
     * </p>
     *
     * @author Gary Buyn
    ...
    class SimpleOpenGLPickingEngine : public RunnableEngine, public PickingEngine
    {
      public:
        ...
        void
        firePickEvent(PickEvent event) const;

        boost::shared_ptr<Camera>
        getCamera() const;

        boost::shared_ptr<Picker>
        getPicker() const;
        ...
        void
        pick(const float x, const float y, const float width, const float height);

        void
        pick(const Pick pick);

        void
        pickViewport(const int viewportWidth, const int viewportHeight, const int x, const int y, const int width,
            const int height);

        void
        pickViewport(const int viewportWidth, const int viewportHeight, const Pick pick);

        void
        removePickListener(const PickListener& listener);
        ...
        void
        setPicker(boost::shared_ptr<Picker> picker);
      ...
      private:
         ...
         * </p>
         */
        std::vector<boost::shared_ptr<PickListener> > fListeners;
         ...
         * </p>
         */
        boost::shared_ptr<Picker> fPicker;
         ...
         */
        Pick
        convertPickCoordinatesFromViewportToSceneGraph(const float viewportWidth, const float viewportHeight, Pick pick) const;
    };
  }
}

...
namespace simplicity
{
	...
	const Node& BreadthFirstConstNodeIterator::getNextNode()
	{
		...
		for (shared_ptr<Node> child : nextNode.getChildren())
		{
			if (find_if(queuedNodes.begin(), queuedNodes.end(), AddressEquals<Node>(*child)) == queuedNodes.end())
			{
				queuedNodes.push_back(*child);

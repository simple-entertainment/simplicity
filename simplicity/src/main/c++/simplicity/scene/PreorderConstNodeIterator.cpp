...
{
	PreorderConstNodeIterator::PreorderConstNodeIterator(const Node& root) :
	...
	const Node* PreorderConstNodeIterator::findNextNode()
	{
		backtracksToNextNode = 0;
		...
		vector<shared_ptr<Node> >::iterator iterator = find_if(siblings.begin(), siblings.end(),
					AddressEquals<Node>(*nextNode));
		iterator++;

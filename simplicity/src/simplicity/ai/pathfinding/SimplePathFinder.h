...
namespace simplicity
{
  /**
   * <p>
   * Performs a simple path-finding algorithm that does not take the weight of any connections into account (i.e. it assumes all connections to have
   * an equal weight). The algorithm is loosly based on one found on Wikipedia: http://en.wikipedia.org/wiki/Pathfinding.
   * </p>
   *
   * @author Gary Buyn
   */
  class SimplePathFinder : public PathFinder
  {
    public:
      ...
      std::vector<boost::shared_ptr<const Node> >
      findShortestPath();
    ...
    private:
      /**
       * <p>
       * The {@link simplicity::Node Node}s that are at the boundary of the search after the last step that was performed.
       * </p>
       */
      std::vector<boost::shared_ptr<const Node> > boundaryNodes;
       ...
       */
      void
      markAsTraversed(const Node& node);
  };
}
